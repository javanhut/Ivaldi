...
impl GitHubClient {
    ...
    pub fn list_branches(&self, owner: &str, repo: &str) -> Result<Vec<BranchInfo>, GitHubError> {
        ...
        let mut page = 1u32;
        loop {
            let resp = self.req("GET", &format!("/repos/{}/{}/branches?per_page=100&page={}", owner, repo, page))
                .call().map_err(gh_err)?;
    ...
    pub fn get_tree(&self, owner: &str, repo: &str, sha: &str) -> Result<TreeResponse, GitHubError> {
        let resp = self.req("GET", &format!("/repos/{}/{}/git/trees/{}?recursive=1", owner, repo, sha))
            .call().map_err(gh_err)?;
        resp.into_json().map_err(gh_err)
    }

    pub fn list_commits(&self, owner: &str, repo: &str, branch: &str, depth: usize) -> Result<Vec<CommitInfo>, GitHubError> {
        let mut all = Vec::new();
        let mut page = 1u32;
        loop {
            let resp = self.req("GET", &format!("/repos/{}/{}/commits?sha={}&per_page=100&page={}", owner, repo, branch, page))
                .call().map_err(gh_err)?;
            let batch: Vec<CommitInfo> = resp.into_json().map_err(gh_err)?;
            let n = batch.len();
            all.extend(batch);
            if depth > 0 && all.len() >= depth { all.truncate(depth); break; }
            if n < 100 { break; }
            page += 1;
        }
        Ok(all)
    ...
    pub fn download_file(&self, owner: &str, repo: &str, path: &str, git_ref: &str) -> Result<Vec<u8>, GitHubError> {
        let url = format!("https://raw.githubusercontent.com/{}/{}/{}/{}", owner, repo, git_ref, path);
        let resp = self.req("GET", &url).call().map_err(gh_err)?;
        let mut buf = Vec::new();
        resp.into_reader().read_to_end(&mut buf).map_err(|e| GitHubError::Other(e.to_string()))?;
        Ok(buf)
    }

    pub fn create_blob(&self, owner: &str, repo: &str, content: &[u8]) -> Result<String, GitHubError> {
        let encoded = base64::engine::general_purpose::STANDARD.encode(content);
        let body = serde_json::json!({"content": encoded, "encoding": "base64"});
        let resp = self.req("POST", &format!("/repos/{}/{}/git/blobs", owner, repo))
            .send_json(body).map_err(gh_err)?;
        let r: ShaResponse = resp.into_json().map_err(gh_err)?;
        Ok(r.sha)
    }

    pub fn create_tree(&self, owner: &str, repo: &str, entries: Vec<TreeEntryCreate>, base_tree: Option<&str>) -> Result<String, GitHubError> {
        let mut body = serde_json::json!({"tree": entries});
        if let Some(b) = base_tree { body["base_tree"] = serde_json::Value::String(b.into()); }
        let resp = self.req("POST", &format!("/repos/{}/{}/git/trees", owner, repo))
            .send_json(body).map_err(gh_err)?;
        let r: ShaResponse = resp.into_json().map_err(gh_err)?;
        Ok(r.sha)
    }

    pub fn create_commit(&self, owner: &str, repo: &str, message: &str, tree_sha: &str, parents: &[String]) -> Result<String, GitHubError> {
        let body = serde_json::json!({"message": message, "tree": tree_sha, "parents": parents});
        let resp = self.req("POST", &format!("/repos/{}/{}/git/commits", owner, repo))
            .send_json(body).map_err(gh_err)?;
        let r: ShaResponse = resp.into_json().map_err(gh_err)?;
        Ok(r.sha)
    }

    pub fn update_ref(&self, owner: &str, repo: &str, branch: &str, sha: &str, force: bool) -> Result<(), GitHubError> {
        let body = serde_json::json!({"sha": sha, "force": force});
        self.req("PATCH", &format!("/repos/{}/{}/git/refs/heads/{}", owner, repo, branch))
            .send_json(body).map_err(gh_err)?;
        Ok(())
    }

    pub fn create_ref(&self, owner: &str, repo: &str, branch: &str, sha: &str) -> Result<(), GitHubError> {
    ...
    pub fn request_device_code() -> Result<DeviceCodeResponse, GitHubError> {
        let client_id = std::env::var("IVALDI_GITHUB_CLIENT_ID").unwrap_or(auth::GITHUB_CLIENT_ID.into());
        ...
        let resp = ureq::post(auth::GITHUB_DEVICE_CODE_URL)
            ...
            .send_string(&body).map_err(gh_err)?;
        resp.into_json().map_err(gh_err)
    }
    ...
    pub fn poll_for_token(device_code: &str, interval: u64) -> Result<Token, GitHubError> {
        ...
        loop {
            ...
            let body = format!("client_id={}&device_code={}&grant_type=urn:ietf:params:oauth:grant-type:device_code", client_id, device_code);
            ...
            match r.error.as_deref() {
                ...
                Some("slow_down") => { thread::sleep(Duration::from_secs(5)); continue; }
                Some(e) => return Err(GitHubError::Other(format!("{}: {}", e, r.error_description.unwrap_or_default()))),
                None => continue,
            }
        }
    }
...
pub struct RepoInfo { pub name: String, pub full_name: String, #[serde(default)] pub description: Option<String>, pub private: bool, pub default_branch: String }

#[derive(Debug, Deserialize)]
pub struct BranchInfo { pub name: String, pub commit: BranchCommit }
#[derive(Debug, Deserialize)]
pub struct BranchCommit { pub sha: String }

#[derive(Debug, Deserialize)]
pub struct CommitInfo { pub sha: String, pub commit: CommitDetail, #[serde(default)] pub parents: Vec<ParentRef> }
#[derive(Debug, Deserialize)]
pub struct CommitDetail { pub message: String, pub author: AuthorInfo, pub tree: TreeRef }
#[derive(Debug, Deserialize)]
pub struct AuthorInfo { pub name: String, pub email: String, #[serde(default)] pub date: Option<String> }
#[derive(Debug, Deserialize)]
pub struct TreeRef { pub sha: String }
#[derive(Debug, Deserialize)]
...
pub struct TreeEntryCreate { pub path: String, pub mode: String, #[serde(rename = "type")] pub entry_type: String, pub sha: String }

#[derive(Debug, Deserialize)]
struct ShaResponse { sha: String }

#[derive(Debug, Deserialize)]
pub struct DeviceCodeResponse { pub device_code: String, pub user_code: String, pub verification_uri: String, pub expires_in: u64, pub interval: u64 }

#[derive(Debug, Deserialize)]
struct TokenPollResponse { access_token: Option<String>, token_type: Option<String>, scope: Option<String>, error: Option<String>, error_description: Option<String> }
...
fn gh_err(e: impl std::fmt::Display) -> GitHubError {
    let m = e.to_string();
    if m.contains("401") { GitHubError::AuthRequired }
    else if m.contains("403") && m.contains("rate") { GitHubError::RateLimited }
    else { GitHubError::Http(m) }
}
...
mod tests {
    ...
    fn tree_entry_create_serializes() {
        let e = TreeEntryCreate { path: "a.txt".into(), mode: "100644".into(), entry_type: "blob".into(), sha: "abc".into() };
